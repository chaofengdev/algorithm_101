...
## BM40 重建二叉树/前序中序序列构建二叉树

```java
//官方解答：递归--这题的官方题解论简单程度完爆市面上大多数的递归算法。
//但其实有大量的优化空间，比如说每次都要遍历中序的序列，另外就是需要反复复制数组。
//这两点对效率影响很大很大，但是谁让他简单呢？
//理解这题的递归，再做由前序和中序求二叉树的后序这类题，手到擒来。
import java.util.*;
/**
...
public class Solution {
    public TreeNode reConstructBinaryTree(int [] pre,int [] vin) {
        if(pre.length == 0 || vin.length == 0) return null;//空树返回null
        TreeNode root = new TreeNode(pre[0]);//从先序序列找根节点
        for(int i = 0; i < vin.length; i++) {//遍历中序序列，找到根节点
            if(vin[i] == pre[0]) {//在中序序列中找到了根节点，位置是数组下标i
                //左子树的区间：先序序列[1,i] 中序序列[0,i-1]
                root.left = reConstructBinaryTree(Arrays.copyOfRange(pre,1,i+1),Arrays.copyOfRange(vin,0,i));
                //右子树的区间：先序序列[i+1,pre.length] 中序序列[i+1,vin.length]
...
//官方解答改进：1.在原数组上建立结点，不需要反复复制数组；2.不需要每次都遍历中序序列，只需要事先遍历一遍map集合保存下标即可。
//下面的是leetcode的官方题解，没有再手写一遍，直接搬过来了。考虑这些效率问题，感觉难度瞬间上升了很多。
/**
...
class Solution {
    ...
    public TreeNode buildTree(int[] preorder, int[] inorder) {
        ...
        //Map<Integer,Integer> map = new HashMap<>();
        for(int i = 0; i < n; i++){
            map.put(inorder[i],i);//保存数组元素与下标的映射
        }
    ...
    public TreeNode buildTree(int[] preorder, int[] inorder,int preLeft, int preRight, int inLeft, int inRight){
        ...
        //最小规模逻辑
        int rootVal = preorder[preLeft];//先序中根节点值
        TreeNode root = new TreeNode(rootVal);//创建根节点
        int pIndex = map.get(rootVal);//先序根节点值对应的中序下标
        root.left = buildTree(preorder,inorder,preLeft + 1,pIndex - inLeft + preLeft,inLeft,pIndex - 1);//遍历左子树，连接到根节点
        root.right = buildTree(preorder,inorder,pIndex - inLeft + preLeft + 1,preRight, pIndex + 1,inRight);//遍历右子树，连接到根节点
        //返回值
        return root;//返回根节点
    }
}
...
```java
//先序递归+层序遍历
//这题算是两道题的结合：先递归构建二叉树，之后层序遍历二叉树，用集合保存每层最后一个结点值。我这个写法应该是最简单易懂的。
//这题官方给出了两个方法，简单看了一下，主要是将递归构建二叉树进行了改进，即BM40所说的两个问题。另外还提供了递归求右视图的方法？我表示真的有点六，做人大可不必如此完美。懒癌发作，懒得看具体递归求右视图的方法怎么写了。
import java.util.*;
public class Solution {
    /**
     * 代码中的类名、方法名、参数名已经指定，请勿修改，直接返回方法规定的值即可
    ...
    public int[] solve (int[] xianxu, int[] zhongxu) {
        // write code here
        //构建二叉树，返回根节点
        TreeNode root = buildTree(xianxu,zhongxu);//这参数命名给我整笑了，这么滑稽的吗？
        //层序遍历二叉树，返回每层最后一个结点，保存在结果集合中
    ...
    //层序遍历二叉树，结果集合保存每层最后一个结点值
    public List<Integer> levelOrder(TreeNode root) {
        if(root == null) return null;//判空
        Queue<TreeNode> queue = new ArrayDeque<>();
        //List<List<Integer>> lists = new ArrayList<>();//本题不需要保存所有层所有结点值
        List<Integer> res = new ArrayList<>();//结果集合，用来保存每一层最后一个结点值
        queue.offer(root);
        while(!queue.isEmpty()) {
            int len = queue.size();
            //List<Integer> list = new ArrayList<>();//本题不需要保存每一层所有结点值
            for(int i = 0; i < len; i++) {
                TreeNode node = queue.poll();
                if(i == len - 1) {//每一层的最后一个结点
                    res.add(node.val);//结点值加入结果集合
                }  
                if(node.left != null) queue.offer(node.left);
                if(node.right != null) queue.offer(node.right);
            }
